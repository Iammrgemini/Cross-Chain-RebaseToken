...
contract CrossChainTest is Test {
    ...
    address Mrgemini = makeAddr("Mrgemini");
    ...
    function setUp() public {
        ...
        arbSepoliaFork = vm.createFork("linea-sepolia");
    ...
    function configureTokenPool(
        ...
        chains[0] = TokenPool.ChainUpdate({
            ...
            allowed: true, // Missing field (needed based on the struct)
            remotePoolAddress: abi.encode(address(remotePool)), // Change to `bytes` instead of `bytes[]`
    ...
    function bridgeTokens(
        ...
        Client.EVM2AnyMessage memory message = Client.EVM2AnyMessage({
            receiver: abi.encode(Mrgemini), // we need to encode the address to bytes
        ...
        ccipLocalSimulatorFork.requestLinkFromFaucet(
            Mrgemini, IRouterClient(localNetworkDetails.routerAddress).getFee(remoteNetworkDetails.chainSelector, message)
        );
        vm.startPrank(Mrgemini);
        ...
        uint256 balanceBeforeBridge = IERC20(address(localToken)).balanceOf(Mrgemini);
        ...
        uint256 sourceBalanceAfterBridge = IERC20(address(localToken)).balanceOf(Mrgemini);
        ...
        uint256 initialArbBalance = IERC20(address(remoteToken)).balanceOf(Mrgemini);
        console.log("Remote balance before bridge: %d", initialArbBalance);
        ccipLocalSimulatorFork.switchChainAndRouteMessage(remoteFork);

        console.log("Remote user interest rate: %d", remoteToken.getUserInterestRate(Mrgemini));
        uint256 destBalance = IERC20(address(remoteToken)).balanceOf(Mrgemini);
    ...
    function testBridgeAllTokens() public {
        ...
        vm.deal(Mrgemini, SEND_VALUE);
        vm.startPrank(Mrgemini);
        // Deposit to the vault and receive tokens
        Vault(payable(address(vault))).deposit{value: SEND_VALUE}();
        // bridge the tokens
        console.log("Bridging %d tokens", SEND_VALUE);
        uint256 startBalance = IERC20(address(sourceRebaseToken)).balanceOf(Mrgemini);
    ...
    function testBridgeAllTokensBack() public {
        ...
        vm.selectFork(arbSepoliaFork);
        console.log("User Balance Before Warp: %d", destRebaseToken.balanceOf(Mrgemini));
        vm.warp(block.timestamp + 3600);
        console.log("User Balance After Warp: %d", destRebaseToken.balanceOf(Mrgemini));
        uint256 destBalance = IERC20(address(destRebaseToken)).balanceOf(Mrgemini);
    ...
    function testBridgeTwice() public {
        ...
        vm.selectFork(sepoliaFork);
        vm.warp(block.timestamp + 3600);
        uint256 newSourceBalance = IERC20(address(sourceRebaseToken)).balanceOf(Mrgemini);
        // bridge the tokens
