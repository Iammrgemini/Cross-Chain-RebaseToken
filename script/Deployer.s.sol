...
contract VaultDeployer is Script{
    function run (address _rebaseToken) public returns (Vault _vault) {
        vm.startBroadcast();
        _vault = new Vault(IRebaseToken(_rebaseToken));
        IRebaseToken(_rebaseToken).grantMintAndBurnRole(address(_vault));
        vm.stopBroadcast();
    }
}